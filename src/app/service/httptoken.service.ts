...
export class HttpTokenService {
    ...
    public get(url: string, showSpinner = true): Observable<any> {
        ...
        return this.http.get(this.getFullUrl(url), { responseType: 'json', headers: this.noCacheRequestOptions() })
                    .pipe(
                        ...
                        finalize(() => { this.hideLoader(showSpinner); })
                    );
    }
    ...
    public post(url: string, body: any, showSpinner = true): Observable<any> {
        ...
        return this.http.post(this.getFullUrl(url), body, { responseType: 'json', headers })
                    .pipe(
                        catchError(this.handleError), // then handle the error
                        finalize(() => { this.hideLoader(showSpinner); })
                    );
    ...
    public put(url: string, body: any, showSpinner = true): Observable<any> {
        ...
        return this.http.put(this.getFullUrl(url), body, { responseType: 'json', headers })
                    .pipe(
                        catchError(this.handleError), // then handle the error
                        finalize(() => { this.hideLoader(showSpinner); })
                    );
    ...
    public delete(url: string, showSpinner = true): Observable<any> {
        this.showLoader(showSpinner);
        const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
        return this.http.delete(this.getFullUrl(url), { responseType: 'json', headers })
                    .pipe(
                        catchError(this.handleError), // then handle the error
                        finalize(() => { this.hideLoader(showSpinner); })
                    );
